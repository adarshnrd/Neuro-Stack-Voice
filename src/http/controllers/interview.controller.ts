...
class InterviewController {
  async start(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const session = await interviewService.startSession(userId, techStack, model, questionsCount, {
        jobDescription,
        userApiKey,
      });
  ...
  async extend(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const session = await interviewService.extendSession(sessionId, userId, additionalCount);
